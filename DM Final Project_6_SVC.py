...
print('Coefficients Confusion SVC - best parameters')
print('tn', tn, 'fp', fp, 'fn', fn, 'tp', tp)

# OUTPUT TO INPUT CAPSTONE PROJECT
# OUTPUT THE MODEL
import pickle
with open('clf.SVC', 'wb') as f:
    pickle.dump(gs, f)

# OUTPUT THE PREDICTIONS
from numpy import savetxt
savetxt('SVC_y_pred.csv', y_pred, delimiter=',')
...
for i in range(len(y_pred)):
    ...
    y_p = y_pred[i]
    if y_t != y_p:
        SVC_x_wrong.append(i)
        SVC_y_wrong.append(y_p)
        print(i, 'predicted:', y_p, 'true:', y_t)

# save to csv file
savetxt('SVC_x_wrong.csv', SVC_x_wrong, delimiter=',')
savetxt('SVC_y_wrong.csv', SVC_y_wrong, delimiter=',')
savetxt('SVC_y_test.csv', y_test, delimiter=',')
savetxt('SVC_y_pred.csv', y_pred, delimiter=',')
