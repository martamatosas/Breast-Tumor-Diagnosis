...
print('Coefficients Confusion RF - best parameters')
print('tn', tn, 'fp', fp, 'fn', fn, 'tp', tp)

# OUTPUT TO INPUT CAPSTONE PROJECT
# OUTPUT THE MODEL
import pickle
with open('clf.RF', 'wb') as f:
    pickle.dump(gs, f)

# OUTPUT THE PREDICTIONS
from numpy import savetxt
savetxt('RF_y_pred.csv', y_pred, delimiter=',')
...
for i in range(len(y_pred)):
    ...
    if y_t != y_p:
        RF_x_wrong.append(i)
        RF_y_wrong.append(y_p)
        print(i, 'predicted:', y_p, 'true:', y_t)

# save to csv file
savetxt('RF_x_wrong.csv', RF_x_wrong, delimiter=',')
savetxt('RF_y_wrong.csv', RF_y_wrong, delimiter=',')
savetxt('RF_y_test.csv', y_test, delimiter=',')
savetxt('RF_y_pred.csv', y_pred, delimiter=',')
